...
const AIHealthAssistant = () => {
  ...
  const handleUserMessage = async () => {
    ...
    if (
      ...
      // Fetch health status if logged in
      fetchHealthStatus(loggedInUser);
      return;
    }
  ...
  const fetchMedicineData = async (diseaseName) => {
    try {
      ...
      setMessages((prev) => [...prev, { sender: "bot", type: "text", text: formatText(text) }]);
  ...
  const fetchSSNData = async (ssn) => {
    try {
      ...
      const regionRes = await axios.get(
        `http://localhost:5001/medical_list?location=${encodeURIComponent(
          Location
        )}`
      );
      ...
      if (regionHospitals.length > 0) {
        ...
        regionHospitals.forEach((facility) => {
          ...
          if (name) {
            ...
            message += link
              ? ` <a href="${link}" target="_blank">Visit</a><br/>`
              : "<br/>";
          }
        });
      }
  ...
  return (
    <div className="ai-health-assistant">
      ...
      {/* Chatbox displaying all messages */}
